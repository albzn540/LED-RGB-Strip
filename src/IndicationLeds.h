...
unsigned long startIndication    = 0;
...
void writeIndicationLedsCallback() {
  if(millis() - startIndication > indicationDuration) {
    writeIndicationLeds();
  } else {
...
void indicateIndex(uint8_t index) {
  indicateIndex(index, CRGB::DeepPink);
}
